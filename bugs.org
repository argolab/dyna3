...
2. run-iterator is calling simplify after it binds each variable.  This is
...
3. disjunct-op needs to allow for taking inner disjuncts which are returned and
...
4. ~$free~ when there is some value used in ~$memo~, the value will be passed.
