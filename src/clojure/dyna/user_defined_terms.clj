...
(defn- empty-user-defined-term [name]
  {:term-name name ;; The name that is associated with this term

   :def-assumption (make-assumption) ;; the assumption that is invalidated anytime that the initial user definition is changed
   :rexprs () ;; R-exprs for the user's definition (these have to get merged together)

   :is-not-memoized-null (make-assumption) ;; assumptions which track different modes for memoization.  A NULL memo means that the memo table _MUST_ be read.  So anything that embeds the R-expr directly is going to depend on the not-memoized-null assumption
   :is-memoized-null (make-invalid-assumption)
   :is-memoized-unk (make-invalid-assumption)

   :memoization-modes #{}

   :memoized-rexpr-assumption (make-invalid-assumption)
   :memoized-rexpr nil ;; if the term is memoized, then this is the R-expr which should be returned.  Will depend on the memoized-rexpr-assumption


   :is-macro false ;; if this a macro, then the ast-to-rexpr will generate a different call structure, in that the returne value from this will get evaluated
   ...
   ;; Raw (uncombined) -> combined (into single R-expr) -> optimized -> compiled -> memo (Last R-expr which is returned)
   :combined-rexpr-assumption (make-invalid-assumption)
   :combined-rexpr nil ;; the combined R-expr

   :optimized-rexpr-assumption (make-invalid-assumption)
   :optimized-rexpr nil ;; if the R-expr has been optimized, then this will cache the result of that here

   :compiled-rexpr-assumption (make-invalid-assumption)
   :compiled-rexpr nil  ;; the compiled version of the R-expr generated by the JIT


   :imported-from-another-file nil ;; if this imported, then this will be the name in the other file.  The lookup process will recurse to looking in the other file

   :is-constraint (make-invalid-assumption)
   :constraint-variable-functional-dependencies {}  ;; if this is a constraint, then there will likely be variable functional dependencies
   })

#_(defn- empty-user-defined-term [name]
...
;; used by memoization & ast-to-rexpr
(defn update-user-term! [name function]
  (let [[old new](swap-vals! (tlocal system/user-defined-terms)
        ...
        oldt (get old name)
        newt (get new name)]
    ;; TODO: check that the proper assumptions are invalidated
  ...
  (let [ut (get-user-term name)]
    ...
    (let [existing-parent-args (get parent-call-arguments name #{})
      ...
      (when (and ut (not (contains? amapped local-map)))
        ...
        ;; then we are going to construct the R-expr for the body, and rename variables as needed
